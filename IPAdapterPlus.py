...
DIR_HOME = os.environ.get('HOME', '/root')
DIR_GITHUB = DIR_HOME + '/GITHUB'
DIR_IPADAPTER = DIR_GITHUB + '/aravind-h-v/IP-Adapter'
sys.path.append(DIR_IPADAPTER)
...
from ip_adapter.ip_adapter import MLPProjModel_aravind
...
MODELS_DIR = GLOBAL_MODELS_DIR if os.path.isdir(
    GLOBAL_MODELS_DIR) else os.path.join(
...
folder_paths.folder_names_and_paths["ipadapter"] = (
    current_paths, folder_paths.supported_pt_extensions)
...
class MLPProjModel_aravind(torch.nn.Module):
...
class MLPProjModelFaceId(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.proj = torch.nn.Sequential(
            ...
            torch.nn.GELU(),
            torch.nn.Linear(id_embeddings_dim * 2,
...
class To_KV(nn.Module):
    ...
    def __init__(self, state_dict):
        ...
        for key, value in state_dict.items():
            self.to_kvs[key.replace(".weight", "").replace(
                ".", "_")] = nn.Linear(value.shape[1],
                                       value.shape[0],
                                       bias=False)
            self.to_kvs[key.replace(".weight",
                                    "").replace(".", "_")].weight.data = value
...
def set_model_patch_replace(model, patch_kwargs, key):
    ...
    if key not in to["patches_replace"]["attn2"]:
        to["patches_replace"]["attn2"][key] = CrossAttentionPatch(
            **patch_kwargs)
    else:
...
def masked_tiling(image, short_side_tiles, weight=0.6, blur=0):
    ...
    ref_image = F.interpolate(image.permute([0, 3, 1, 2]),
    ...
    for i in range(num_tiles_y):
        for j in range(num_tiles_x):
            ...
            tile = ref_image[:1, start_height:end_height,
                             start_width:end_width, :]
            tiles.append(tile.squeeze(0))

            # create mask
            mask = torch.zeros([new_height, new_width],
                               dtype=image.dtype,
                               device=image.device)
    ...
    if num_tiles_x != 1 and num_tiles_y != 1:
        ...
        if image.shape[1:3] != comp_tiles.shape[1:3]:
            comp_tiles = F.interpolate(comp_tiles.permute([0, 3, 1, 2]),
                                       size=(image.shape[1], image.shape[2]),
                                       mode="bicubic").permute([0, 2, 3, 1])
        image = torch.cat([comp_tiles, image], dim=0)

        if attn_mask.shape[1:3] != comp_attn_mask.shape[1:3]:
            comp_attn_mask = F.interpolate(comp_attn_mask.unsqueeze(1),
                                           size=(attn_mask.shape[1],
                                                 attn_mask.shape[2]),
                                           mode="bicubic").squeeze(1)
        attn_mask = torch.cat([comp_attn_mask, attn_mask], dim=0)

    if blur > 0:
        attn_mask = TT.GaussianBlur(int(6 * blur + 1), blur)(
            attn_mask.unsqueeze(1)).permute([0, 2, 3, 1]).squeeze(-1)
...
def image_add_noise(image, noise):
    ...
    transforms = TT.Compose([
        ...
        TT.Resize((224, 224),
                  ...
                  antialias=True),
        TT.ElasticTransform(alpha=75.0,
                            sigma=noise * 3.5),  # shuffle the image
        TT.RandomVerticalFlip(
            p=1.0),  # flip the image to change the geometry even more
...
class IPAdapter(nn.Module):
    ...
    def __init__(
        ...
        if is_faceid:
            ...
            self.image_proj_model = self.init_proj_aravind()
        else:
    ...
    @torch.inference_mode()
    def get_image_embeds_faceid_plus(self, face_embed, clip_embed, s_scale,
                                     shortcut):
        embeds = self.image_proj_model(face_embed,
                                       clip_embed,
                                       scale=s_scale,
                                       shortcut=shortcut)
        return embeds
...
class CrossAttentionPatch:
    ...
    def __call__(self, n, context_attn2, value_attn2, extra_options):
        ...
        sigma = extra_options["sigmas"][
            0] if 'sigmas' in extra_options else None
        sigma = sigma.item() if sigma is not None else 999999999.9

        # extra options for AnimateDiff
        ad_params = extra_options[
            'ad_params'] if "ad_params" in extra_options else None
        ...
        for weight, cond, uncond, ipadapter, mask, weight_type, sigma_start, sigma_end, unfold_batch in zip(
                self.weights, self.conds, self.unconds, self.ipadapters,
                self.masks, self.weight_type, self.sigma_start, self.sigma_end,
                self.unfold_batch):
            ...
            if unfold_batch and cond.shape[0] > 1:
                ...
                if ad_params is not None and ad_params["sub_idxs"] is not None:
                    ...
                    # otherwise, need to do more to get proper sub_idxs masks
                        ...
                        if cond.shape[0] < ad_params["full_length"]:
                            cond = torch.cat((cond, cond[-1:].repeat(
                                (ad_params["full_length"] - cond.shape[0], 1,
                                 1))),
                                             dim=0)
                            uncond = torch.cat((uncond, uncond[-1:].repeat(
                                (ad_params["full_length"] - uncond.shape[0], 1,
                                 1))),
                                               dim=0)
                ...
                if cond.shape[0] < batch_prompt:
                    cond = torch.cat((cond, cond[-1:].repeat(
                        (batch_prompt - cond.shape[0], 1, 1))),
                                     dim=0)
                    uncond = torch.cat((uncond, uncond[-1:].repeat(
                        (batch_prompt - uncond.shape[0], 1, 1))),
                                       dim=0)
                ...
                k_cond = ipadapter.ip_layers.to_kvs[self.k_key](cond).repeat(
                    batch_prompt, 1, 1)
                k_uncond = ipadapter.ip_layers.to_kvs[self.k_key](
                    uncond).repeat(batch_prompt, 1, 1)
                v_cond = ipadapter.ip_layers.to_kvs[self.v_key](cond).repeat(
                    batch_prompt, 1, 1)
                v_uncond = ipadapter.ip_layers.to_kvs[self.v_key](
                    uncond).repeat(batch_prompt, 1, 1)

            if weight_type.startswith("linear"):
                ip_k = torch.cat([(k_cond, k_uncond)[i]
                                  for i in cond_or_uncond],
                                 dim=0) * weight
                ip_v = torch.cat([(v_cond, v_uncond)[i]
                                  for i in cond_or_uncond],
                                 dim=0) * weight
            else:
                ip_k = torch.cat([(k_cond, k_uncond)[i]
            ...
            out_ip = optimized_attention(q, ip_k, ip_v,
            ...
            if mask is not None:
                ...
                if (mask.shape[0] > 1 and ad_params is not None
                    ...
                    # otherwise, need to do more to get proper sub_idxs masks
                        ...
                        if mask_downsample.shape[0] < ad_params["full_length"]:
                            mask_downsample = torch.cat(
                                (mask_downsample, mask_downsample[-1:].repeat(
                                    (ad_params["full_length"] -
                                     mask_downsample.shape[0], 1, 1))),
                                dim=0)
                        # if we have too many remove the excess (should not happen, but just in case)
                        if mask_downsample.shape[0] > ad_params["full_length"]:
                            mask_downsample = mask_downsample[:ad_params[
                                "full_length"]]
                ...
                # otherwise, perform usual mask interpolation
                    ...
                    mask_downsample = F.interpolate(mask.unsqueeze(1),
                                                    size=(mask_h, mask_w),
                                                    mode="bicubic").squeeze(1)
                ...
                # repeat the masks
                mask_downsample = mask_downsample.repeat(
                    len(cond_or_uncond), 1, 1)
                mask_downsample = mask_downsample.view(
                    mask_downsample.shape[0], -1,
                    1).repeat(1, 1, out.shape[2])
...
class IPAdapterModelLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "ipadapter_file":
                (folder_paths.get_filename_list("ipadapter"), )
            }
        }
        ...
        if ckpt_path.lower().endswith(".safetensors"):
            ...
            for key in model.keys():
                if key.startswith("image_proj."):
                    st_model["image_proj"][key.replace("image_proj.",
                                                       "")] = model[key]
                elif key.startswith("ip_adapter."):
                    st_model["ip_adapter"][key.replace("ip_adapter.",
                                                       "")] = model[key]
            model = st_model
...
class IPAdapterApply:
    ...
    def apply_ipadapter(
        ...
        self.dtype = torch.float16 if comfy.model_management.should_use_fp16(
        ) else torch.float32
        ...
        if is_tiled:
            image, attn_mask = masked_tiling(image, short_side_tiles,
                                             tile_weight, tile_blur)

        if self.is_faceid and not insightface:
            raise Exception('InsightFace must be provided for FaceID models.')

        output_cross_attention_dim = ipadapter["ip_adapter"][
        ...
        if embeds is not None:
            ...
            if self.is_faceid:
                insightface.det_model.input_size = (
                    640, 640)  # reset the detection size
                face_img = tensorToNP(image)
                face_embed = []
                face_clipvision = []

                for i in range(face_img.shape[0]):
                    for size in [(size, size)
                        ...
                        if face:
                            face_embed.append(
                                torch.from_numpy(
                                    face[0].normed_embedding).unsqueeze(0))
                            face_clipvision.append(
                                NPToTensor(
                                    insightface_face_align.norm_crop(
                                        face_img[i],
                                        landmark=face[0].kps,
                                        image_size=256)))
                ...
                neg_image = image_add_noise(image,
                                            noise) if noise > 0 else None
                ...
                if self.is_plus:
                    ...
                    if noise > 0:
                        clip_embed_zeroed = clip_vision.encode_image(
                            neg_image).penultimate_hidden_states
                    else:
                ...
                clip_embed = encode_image_masked(clip_vision, image,
                                                 clip_vision_mask)
                neg_image = image_add_noise(image,
                                            noise) if noise > 0 else None
        ...
        sigma_start = self.work_model.model.model_sampling.percent_to_sigma(
            start_at)
        sigma_end = self.work_model.model.model_sampling.percent_to_sigma(
            end_at)

        if is_tiled:
            for i in range(
                    image_prompt_embeds.shape[0]
            ):  #TODO: check if we can apply one mask per image in the attention patch phase
                patch_kwargs = {
                    "number": 0,
                    "weight": self.weight,
    ...
    def apply_patch(self, patch_kwargs):
        if not self.is_sdxl:
            for id in [1, 2, 4, 5, 7,
                       8]:  # id of input_blocks that have cross attention
                set_model_patch_replace(self.work_model, patch_kwargs,
                                        ("input", id))
                patch_kwargs["number"] += 1
            for id in [3, 4, 5, 6, 7, 8, 9, 10,
                       11]:  # id of output_blocks that have cross attention
                set_model_patch_replace(self.work_model, patch_kwargs,
                                        ("output", id))
                patch_kwargs["number"] += 1
            set_model_patch_replace(self.work_model, patch_kwargs,
            ...
            for id in [4, 5, 7,
                       8]:  # id of input_blocks that have cross attention
                block_indices = range(2) if id in [4, 5] else range(
                    10)  # transformer_depth
                for index in block_indices:
                    set_model_patch_replace(self.work_model, patch_kwargs,
                                            ("input", id, index))
                    patch_kwargs["number"] += 1
            for id in range(
                    6):  # id of output_blocks that have cross attention
                block_indices = range(2) if id in [3, 4, 5] else range(
                    10)  # transformer_depth
                for index in block_indices:
                    set_model_patch_replace(self.work_model, patch_kwargs,
                                            ("output", id, index))
...
def prepImage(image,
    ...
    if "pad" in crop_position:
        ...
        output = F.pad(output, (pad_l, pad_r, pad_t, pad_b),
                       value=0,
                       mode="constant")
    else:
        crop_size = min(oh, ow)
    ...
    if padding > 0:
        output = F.pad(output, (padding, padding, padding, padding),
                       value=255,
                       mode="constant")
...
class PrepImageForInsightFace:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("IMAGE", ),
                "crop_position": (["center", "top", "bottom", "left",
                                   "right"], ),
                "sharpening": ("FLOAT", {
                    "default": 0.0,
                    "min": 0,
    ...
    def prep_image(self,
        ...
        if pad_around:
            ...
            padding = 0
            size = (640, 640)
        output = prepImage(image, "LANCZOS", crop_position, size, sharpening,
                           padding)
...
class PrepImageForClipVision:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "image": ("IMAGE", ),
                "interpolation": ([
                    "LANCZOS", "BICUBIC", "HAMMING", "BILINEAR", "BOX",
                    "NEAREST"
                ], ),
                "crop_position":
                (["top", "bottom", "left", "right", "center", "pad"], ),
                "sharpening": ("FLOAT", {
                    "default": 0.0,
                    "min": 0,
                    "max": 1,
    ...
    def prep_image(self,
                   ...
                   sharpening=0.0):
        size = (224, 224)
        output = prepImage(image, interpolation, crop_position, size,
                           sharpening, 0)
        return (output, )
    ...
    def preprocess(self,
        ...
        if image_2 is not None:
            if image_1.shape[1:] != image_2.shape[1:]:
                image_2 = comfy.utils.common_upscale(image_2.movedim(-1, 1),
                                                     image.shape[2],
                                                     image.shape[1],
                                                     "bilinear",
                                                     "center").movedim(1, -1)
        ...
        if image_3 is not None:
            if image.shape[1:] != image_3.shape[1:]:
                image_3 = comfy.utils.common_upscale(image_3.movedim(-1, 1),
                                                     image.shape[2],
                                                     image.shape[1],
                                                     "bilinear",
                                                     "center").movedim(1, -1)
        ...
        if image_4 is not None:
            if image.shape[1:] != image_4.shape[1:]:
                image_4 = comfy.utils.common_upscale(image_4.movedim(-1, 1),
                                                     image.shape[2],
                                                     image.shape[1],
                                                     "bilinear",
                                                     "center").movedim(1, -1)
        ...
        if any(e != 1.0 for e in weight):
            weight = torch.tensor(weight).unsqueeze(
                -1) if not ipadapter_plus else torch.tensor(weight).unsqueeze(
                    -1).unsqueeze(-1)
...
class IPAdapterLoadEmbeds:
    ...
    def INPUT_TYPES(s):
        ...
        files = [
            ...
            for root, dirs, files in os.walk(input_dir) for file in files
